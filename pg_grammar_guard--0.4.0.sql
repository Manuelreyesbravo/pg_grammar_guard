-- pg_grammar_guard 0.4.0
--
-- 0.3.0 moves the guard half out. Approving a baseline, detecting drift and
-- telling "changed" from "nobody ever approved it" are not grammar problems --
-- they are the same problem four extensions solved separately -- so they now
-- live in pg_living_assertions and this file keeps what is actually its own:
-- how to build a grammar from the catalog, and how to fingerprint one.
--
...
-- ---------------------------------------------------------------------------
-- IDENTITY. What stays behind when the guard half leaves.
--
-- The registry knows how to store a claim, run it and date it. It does NOT
-- know when two grammars are the same grammar -- that is domain knowledge and
-- it belongs here. This is the honest line between a piece and its consumer:
-- the consumer keeps its own notion of identity and gives up its private copy
-- of baseline, approve, drift and severity.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- THE GUARD HALF IS NOT HERE ANY MORE. It is pg_living_assertions.
--
-- Up to 0.2.0 this file carried its own baseline table, its own approve(), its
-- own drift comparison and its own severity vocabulary (drift|never_approved).
-- So did pg_plan_guard (ok|drifted|error, plus a drift_log), pg_recall_guard
-- (a baselines table and an approve of its own) and pg_promise_guard
-- (breach|gap). Four extensions, four private answers to the same question --
-- which is not a coincidence but the symptom of a missing abstraction.
--
-- WHAT THE MOVE EXPOSED, and it is worth more than the code it deleted:
-- check_grammar(name, fields) required the CALLER to bring the world with them.
-- If you passed a spec built from a stale variable, it happily compared the
-- baseline against something that was not your catalog and reported no drift.
-- A registry that stores the check has no such option: what gets stored is the
-- QUERY THAT REBUILDS THE SPEC FROM THE LIVE CATALOG, so the check can be run
-- by a cron job, by a deploy gate, or by somebody who was not there when it was
-- approved -- and it is looking at the real database every time.
-- ---------------------------------------------------------------------------
-- 0.4.0: the freeze-and-compare is gone from here too.
--
-- 0.3.0 moved the baseline, the approve and the drift to pg_living_assertions
-- and then rebuilt, by hand, the three steps that surround them: evaluate the
-- expression now, freeze it, generate a check that re-evaluates and compares.
-- pg_plan_guard writes those same three steps for plan advice. So the
-- duplication this extension exists to remove had crept back one level up, and
-- a measurement caught it: the saving from the 0.3.0 port came out under what
-- had been declared, which was not the metric being wrong -- it was the port
-- not being finished.
--
-- What is left here is the only thing that is genuinely ours: WHICH expression
-- describes the grammar, and what counts as the same grammar.
