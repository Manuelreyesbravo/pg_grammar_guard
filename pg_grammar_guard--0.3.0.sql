...
DECLARE
    ...
    spec jsonb;
BEGIN
    -- Evaluated once, now, to freeze what you are approving. If the spec query
    -- is wrong this raises HERE, at approval time, rather than being stored and
    -- reported as a broken check forever afterwards.
    ...
    IF spec IS NULL THEN
        RAISE EXCEPTION 'the spec query returned NULL'
            USING HINT = 'p_spec_sql must be a query returning one jsonb value, '
                         'e.g. select jsonb_build_array(grammar_guard.catalog_correlated(...)).';
    END IF;
